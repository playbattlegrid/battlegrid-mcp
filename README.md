...
- **A signal rule cannot be `required` at allocation Off** (32.0.0, `enforce-required-allocation-invariant`). `{ required: true, allocation: 0 }` is now REJECTED on every surface that writes rules — `compile_strategy_plan`, `apply_strategy_plan` and `update_strategy_signal_rule`. A payload accepted under contract 31 can be refused under 32 **without one byte of it changing**, so this note is the only signal you get. The pair never meant anything: the scorecard's triggered set already excludes allocation Off and the Min-Required gate counts a subset of that set, so a Required+Off rule could neither satisfy `minRequiredCount` nor block a trade — it was not a weaker setting, it had no observable effect at all. The refusal is a typed `VALIDATION_ERROR` whose **`details.inertRequiredSignalIds` carries the complete sorted list** of offending signals; the message names a bounded prefix and summarizes the rest, so read the detail rather than parsing the prose — real cases are long (one production strategy held 74). **Nothing is repaired for you**: the allocation is not raised (that would invent a scoring weight you never chose) and the flag is not cleared (that would discard what you authored). Either repair satisfies the boundary; picking one is yours. **Not visible in `tools/list`** — `zod-to-json-schema` drops effects by construction, so no published schema states this and you cannot pre-validate it; the typed error is the whole contract. **Stored rules were not migrated**, so a strategy holding the pair reads back unchanged and is refused on its owner's next write — including a write that touches only its name.
