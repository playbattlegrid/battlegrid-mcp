...
| 27.0.0 | Server contract v27.0.0, **breaking**: the trade-defaults catalog stops serving four seeds for fields no surface can author. `get_trading_config_catalog` drops `defaultMinAtrPct`, `defaultMinStopLossPct`, `defaultMaxStopLossPct` and `defaultMinRiskRewardRatio` from `defaults` — each seeded a per-agent field that `move-trade-level-policy-to-strategy` and the setup-gate move made strategy-owned, so the shared `.strict()` `TradingConfigSchema` already rejected all four as unknown keys and the create-agent form read none of them. The bound strategy owns and materializes those values instead; nothing replaces the keys. `defaultMaxEntryDeviationAtrMultiple` and `defaultTtlMinutes` are unaffected — they are not seeds but the platform values that govern every decision. **No input-acceptance change**, since none of the four was ever an accepted request field: this is an output-only MAJOR, the same call v7.0.0 made. Off the wire the same change destroys the per-agent entry-guard columns v26.0.0 de-referenced and adds `chk_default_ttl_minutes`, so the unfilled-lifetime budget keeps a database backstop; no tool's shape depends on either. Never published as a package version |
