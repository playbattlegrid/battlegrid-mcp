...
- **`apply_strategy_plan` requires the conditions axis.** The plan post-state now carries required `conditions` and `conditionVerdicts`; a payload assembled from the v3 field list is rejected with `plan.conditions: Required`. A client that byte-copies compile's `approvedPlan` into apply carries the axis automatically and needs no change — that is still the intended flow. The proxy itself is unchanged: it embeds no schemas, pins no contract version, and forwards `{ request }` verbatim.
