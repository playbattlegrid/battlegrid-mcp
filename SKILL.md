...
7. **Apply only the exact reviewed plan.**
   - After explicit user approval: `apply_strategy_plan({ request: { approvedPlan, planToken, confirm: true } })`. Resubmit `approvedPlan` exactly as returned — never include `reviewContext` or rebuild a scorecard client-side. Changed configuration reaches every bound agent immediately.
